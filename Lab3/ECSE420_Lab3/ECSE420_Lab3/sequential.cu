...
int main(int argc, char* argv[])
{
	...
	char* nextLevelNodes_filename = "nextLvlNodes_seq.raw";*/
	...
	if (TEST == 1)
	{
		...
		// Run 10 times for time to be more accurate
		for (int k = 0; k < 10; k++) {
		...
		clock_t stop = clock();
		float totalTime = ((stop - start) * 1000 / CLOCKS_PER_SEC / 10.0);
		printf("Sequential time taken: %f (ms)\n", totalTime ); //time in seconds
	}
	else
	{
		printf("no test");
